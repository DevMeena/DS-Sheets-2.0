...
import { getListQuestCount, getListSolvedQuestCount, resetListProgress } from '../admin'
          ...
          <div className="row card-row">
            ...
            <div className="button-container col-md-4">
              <div className="inner-button-container">
                <button type="button" onClick={handleSolveClick} value={id} name="startButton" className="btn btn-success">Start solving</button>
                {/* <form action="/reset-list-progress" method="POST"> */}
                  {/* <input type="text" defaultValue="<%= item.questionSet %>" name="list" hidden /> */}
                  <button type="button" className="btn btn-danger" onClick={resetProgress}>Reset progress</button>
                {/* </form> */}
                {/* <button type="button" className="btn btn-primary" disabled>About this list</button> */}
                <div className='progress-border'>
                    <div className="progress-bar" style={{"width" : `${progress}%` }} > <span className='progress-text'>{progress}%</span> </div>
                </div>
              </div>
            </div>  
          </div>
